...
int main(){
	while(ALWAYS) {
		...
		printf("HI :)\n");
		printf("%s", CYN);
		printf("HERE IS THE PLAYLIST OF MY PROJECT\n");
		printf("PLEASE CHOOSE YOUR MUSIC:\n");
		printf("SORRY FOR THE LACK OF QUALITY OF BEEP AND MY IMPRECISE INFORMATIONS\n");
		printf("PLEASE CHOOSE YOUR MUSIC:\n");
		printf("%s", GRN);
		printf("1.BREAK\n2.FUR ELISE\n3.SUGARPLUM\n4.BACH_VARIATION_22\n");
		...
		scanf(" %c",&input);
			 if (input == ONE) BREAK();
		else if (input == TWO) FUR_ELISE();
		else if (input == THREE) SUGARPLUM();
		else if (input == FOUR) BACH_VARIATION_22();
		else if (input == FIVE) exit(0);
		else {printf("%s", RED); printf("WRONG INPUT\nPLEASE ENTER AN APPROPRIATE NUMBER!"); }
	}
}
...
void BREAK()
{
    ...
    FILE *infile = fopen ("break.mid","rb"); /******* reading bytes if MIDI file in the project folder *******/
////////////////////////////forelise//break//sugarplum//bach22
...
we should convert into integer numbers because if we don't do that, the program prints its ascii code instead of the number! *******/
    ...
    while (flag != FINISH)
    {
                ...
                shape_form++;
                						printf("%s", RED);
                if (shape_form % 8 == 0) printf("\n");
                if (shape_form % 8 == 1) printf("   *******\n");
                if (shape_form % 8 == 2) printf("   *******\n    *****\n");
                if (shape_form % 8 == 3) printf("   *******\n    *****\n     * *\n"); 
                if (shape_form % 8 == 4) printf("   *******\n    *****\n     * *\n      *\n");
                if (shape_form % 8 == 5) printf("   *******\n    *****\n     * *\n      *\n     * *\n");
                if (shape_form % 8 == 6) printf("   *******\n    *****\n     * *\n      *\n     * *\n    *****\n");
                if (shape_form % 8 == 7) printf("   *******\n    *****\n     * *\n      *\n     * *\n    *****\n   *******\n");
...
void FUR_ELISE()
{
    ...
    header_chunk chunk;
    track_information track;
    FILE *infile = fopen ("forelise.mid","rb"); /******* reading bytes if MIDI file in the project folder *******/
////////////////////////////forelise//break//sugarplum//bach22
    if (infile==NULL) { // to check if input file is opened properly or not
        printf("Sorry, input file not found");
        exit(1); // terminates the program
...
we should convert into integer numbers because if we don't do that, the program prints its ascii code instead of the number! *******/
    ...
    while (flag != FINISH)
    {
                ...
                if (event_beep == 255 || event_beep2 == 255)  {
                    ...
                    printf ("tempo in my tempoo_check is %d\n", tempo1);
                    flag = 16;}
                                        if (tempoo_check[1] == 47 || tempoo_check[0] == 47){
                        if (tempoo_check[0] == 47) {fseek ( infile , -3 , SEEK_CUR);
                                                   fread (tempoo_check , sizeof(tempoo_check) , 1 , infile);
...
/////////////////////////////////128 = 0x80 && 159 == 0x9f
                ...
                temp5 = 10000 * chunk.data.division;
                duration = temp5 * time / tempo_dec;
                if (duration < 0 ) duration *= -1 ;/******* just for fun :0  *******/
                                        printf("%s", CYN);
                printf("eve%x\n",event_beep);
                                        printf("%s", GRN);
                frequency = event[1];
                ...
                velocity = fgetc(infile);
                /******* the following function checks and returns the frequency of each character *******/
                temp6 = findfreq(pointer);
                                    printf("%s", YEL);  
                printf ("duration %d\n\n\n", duration);
                temp6 += 20;
                beep (temp6 , duration/6 );
                shape_form++;
                                        printf("%s", BLU);
                if (shape_form % 6 == 0) printf("\n");
                if (shape_form % 6 == 1) printf("    *\n");
                if (shape_form % 6 == 2) printf("    *\n   ***\n");
                if (shape_form % 6 == 3) printf("    *\n   ***\n  *****\n"); 
                if (shape_form % 6 == 4) printf("    *\n   ***\n  *****\n   ***\n");
                if (shape_form % 6 == 5) printf("    *\n   ***\n  *****\n   ***\n    *\n");
                }
                
                /******* this counter was for check ste by step part of file at the beginning *******/
                if (flag == 7 || flag == 11) count++;
...
void SUGARPLUM()
{
    ...
    header_chunk chunk;
    track_information track;
    FILE *infile = fopen ("sugarplum.mid","rb"); /******* reading bytes if MIDI file in the project folder *******/
